// =========================
// 📅 SPIELPLAN GENERIEREN (ID BASED + SAFE)
// =========================
import { game } from "../core/state.js";

// =========================
// 🧠 ID HELPERS (ADD ONLY)
// =========================
function normalizeId(val){
  if(val === null || val === undefined) return null;
  return String(val);
}

function resolveTeamId(team){
  if(!team) return null;

  if(typeof team === "string") return normalizeId(team);
  if(typeof team === "number") return normalizeId(team);

  if(typeof team === "object"){
    return normalizeId(team.id);
  }

  return null;
}

// =========================
// 🧠 HELPERS (EXISTING)
// =========================
function getTeamName(team){
  if(!team) return "Unbekannt";
  if(typeof team === "string") return team;
  return team.name || "Unbekannt";
}

function getTeamId(team){
  if(!team) return null;
  if(typeof team === "string") return null; // 🔥 bleibt (NO-LOSS)
  return normalizeId(team.id || null); // 🔒 FIX
}

function normalizeTeam(team){

  // 👉 Strings bleiben Strings (NO-LOSS)
  if(typeof team === "string"){
    return { name: team };
  }

  // 🔒 ensure ID is string if present
  if(team && team.id !== undefined){
    return {
      ...team,
      id: normalizeId(team.id)
    };
  }

  return team;
}

function normalize(val){
  return String(val || "").toLowerCase().trim();
}

// =========================
// 📅 GENERATE
// =========================
function generateSchedule(){

  const league = game.league?.current;

  if(!league || !Array.isArray(league.teams) || league.teams.length < 2){
    console.error("❌ Ungültige Liga / zu wenig Teams");
    return;
  }

  if(game.league.schedule && game.league.schedule.length > 0){
    console.log("ℹ️ Spielplan existiert bereits → wird nicht neu erstellt");
    return;
  }

  let teamsRaw = league.teams.map(normalizeTeam);

  const seen = new Set();
  let teams = [];

  teamsRaw.forEach(t => {

    const key = resolveTeamId(t) || t.name; // 🔒 FIX

    if(!key){
      console.error("❌ Team ohne Key:", t);
      return;
    }

    if(seen.has(key)){
      console.warn("⚠️ Duplicate Team erkannt:", key);
      return;
    }

    seen.add(key);
    teams.push(t);
  });

  const originalCount = teams.length;

  if(teams.length % 2 !== 0){
    teams.push({ id: "BYE", name: "BYE" });
  }

  const totalRounds = teams.length - 1;
  const half = teams.length / 2;

  const rounds = [];
  let rotation = [...teams];

  // =========================
  // 🔁 HINRUNDE
  // =========================
  for(let r = 0; r < totalRounds; r++){

    const round = [];

    for(let i = 0; i < half; i++){

      const home = rotation[i];
      const away = rotation[rotation.length - 1 - i];

      // 🔒 FIX: stabile ID-Vergleiche
      const homeKey = resolveTeamId(home) || home.name;
      const awayKey = resolveTeamId(away) || away.name;

      if(homeKey === awayKey){
        console.error("❌ Selbstduell verhindert:", homeKey);
        continue;
      }

      if(home.name !== "BYE" && away.name !== "BYE"){
        round.push({
          id: crypto.randomUUID(),

          homeTeamId: resolveTeamId(home), // 🔒 FIX
          awayTeamId: resolveTeamId(away), // 🔒 FIX

          home,
          away,

          result: null,
          _processed: false
        });
      }
    }

    rounds.push(round);

    const fixed = rotation[0];
    const rest = rotation.slice(1);

    rest.unshift(rest.pop());
    rotation = [fixed, ...rest];
  }

  // =========================
  // 🔁 RÜCKRUNDE
  // =========================
  const returnRounds = rounds.map(round =>
    round.map(match => ({
      id: crypto.randomUUID(),

      homeTeamId: normalizeId(match.awayTeamId), // 🔒 FIX
      awayTeamId: normalizeId(match.homeTeamId), // 🔒 FIX

      home: match.away,
      away: match.home,

      result: null,
      _processed: false
    }))
  );

  game.league.schedule = [...rounds, ...returnRounds];

  game.league.schedule.forEach(round => {
    round._simulated = false;
  });

  game.league.currentRound = 0;
  game.league.currentMatchIndex = 0;

  console.log("✅ Spielplan erstellt:", game.league.schedule.length);

  validateSchedule(originalCount);
}

// =========================
// 🧪 VALIDIERUNG
// =========================
function validateSchedule(expectedTeamCount){

  const schedule = game.league.schedule;

  if(!schedule?.length){
    console.error("❌ Kein Spielplan vorhanden");
    return;
  }

  const firstRound = schedule[0];

  const teams = [];

  firstRound.forEach(match => {
    teams.push(normalizeId(match.homeTeamId) || getTeamName(match.home)); // 🔒 FIX
    teams.push(normalizeId(match.awayTeamId) || getTeamName(match.away)); // 🔒 FIX
  });

  const unique = new Set(teams);
  const expectedMatches = Math.floor(expectedTeamCount / 2);

  if(teams.length !== unique.size){
    console.error("❌ DUPLIKATE IM SPIELTAG!");
  }

  if(firstRound.length !== expectedMatches){
    console.error("❌ Falsche Spielanzahl:", firstRound.length);
  } else {
    console.log("✅ Spieltag korrekt:", expectedMatches);
  }
}

// =========================
// 👉 NEXT MATCH (ID SAFE)
// =========================
function nextMatch(){

  const schedule = game.league?.schedule;

  if(!schedule?.length){
    console.warn("❌ Kein Spielplan");
    return null;
  }

  const myTeamId = normalizeId(game.team?.id); // 🔒 FIX

  const myTeamName =
    String(
      game.team?.name ||
      game.team?.selected ||
      ""
    ).toLowerCase().trim();

  let r = game.league.currentRound || 0;
  let m = game.league.currentMatchIndex || 0;

  for(; r < schedule.length; r++){
    for(; m < schedule[r].length; m++){

      const match = schedule[r][m];

      if(match._processed) continue;

      // 🔒 ID FIRST (SAFE)
      if(myTeamId){
        if(
          normalizeId(match.homeTeamId) === myTeamId ||
          normalizeId(match.awayTeamId) === myTeamId
        ){
          game.league.currentRound = r;
          game.league.currentMatchIndex = m;
          return match;
        }
      }

      // 🔥 fallback (NO-LOSS)
      if(myTeamName){

        const homeName = normalize(getTeamName(match.home));
        const awayName = normalize(getTeamName(match.away));

        if(homeName === myTeamName || awayName === myTeamName){
          game.league.currentRound = r;
          game.league.currentMatchIndex = m;
          return match;
        }
      }

      if(!myTeamId && !myTeamName){
        game.league.currentRound = r;
        game.league.currentMatchIndex = m;
        return match;
      }
    }
    m = 0;
  }

  return null;
}

// =========================
// 👉 ADVANCE (UNCHANGED)
// =========================
function advanceSchedule(){

  let roundIndex = game.league.currentRound || 0;
  let matchIndex = game.league.currentMatchIndex || 0;

  const schedule = game.league.schedule;
  if(!schedule?.length) return;

  matchIndex++;

  if(matchIndex >= schedule[roundIndex].length){
    matchIndex = 0;
    roundIndex++;
  }

  game.league.currentRound = roundIndex;
  game.league.currentMatchIndex = matchIndex;
}

// =========================
// 🏁 SEASON END CHECK (UNCHANGED)
// =========================
function isSeasonFinished(){

  const schedule = game.league.schedule;
  if(!schedule?.length) return true;

  return schedule.every(round =>
    round.every(match => match._processed)
  );
}

// =========================
// 📅 RENDER (UNCHANGED)
// =========================
function renderSchedule(){

  const container = document.getElementById("scheduleView");
  if(!container) return;

  const schedule = game.league?.schedule;

  if(!schedule?.length){
    container.innerHTML = "<p>Kein Spielplan vorhanden</p>";
    return;
  }

  const currentRound = game.league.currentRound ?? 0;
  const currentMatch = game.league.currentMatchIndex ?? 0;

  let html = "";

  schedule.forEach((round, rIndex) => {

    html += `<div class="round">
      <h3>Spieltag ${rIndex + 1}</h3>
      <ul style="list-style:none;padding:0;">`;

    round.forEach((match, mIndex) => {

      const isActive =
        rIndex === currentRound &&
        mIndex === currentMatch;

      const home = getTeamName(match.home);
      const away = getTeamName(match.away);

      html += `
        <li style="
          padding:6px;
          margin:2px 0;
          border-radius:4px;
          ${isActive ? "background:#1a1a1a;color:#00ff88;font-weight:bold;" : ""}
        ">
${home} ${match.result ? match.result.home + ":" + match.result.away : "vs"} ${away}
${match._processed ? " ✅" : ""}
        </li>
      `;
    });

    html += `</ul></div>`;
  });

  container.innerHTML = html;
}

// =========================
// 📦 EXPORTS
// =========================
export {
  generateSchedule,
  nextMatch,
  advanceSchedule,
  isSeasonFinished,
  renderSchedule
};

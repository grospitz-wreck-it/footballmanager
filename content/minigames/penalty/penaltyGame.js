...
import { calculateShot, getBallPosition, resolveShot } from './penaltyPhysics.js';

export class PenaltyGame {
  constructor(rootElement, config = {}) {
    this.root = rootElement;
    this.config = mergePenaltyConfig(DEFAULT_PENALTY_CONFIG, config);
  ...
  start() {
    ...
    this.ui.updateScore(this.state.score, this.config.rounds, this.state.round);
    this.ui.setFeedback('Tap or swipe to shoot');
    this.config.hooks.onRoundStart?.({ round: this.state.round, score: this.state.score });
  ...
  reset() {
    ...
    this.config.hooks.onGameEnd?.(this.state.score);
  }
  ...
  handleShot(inputData) {
    if (!this.state.active || this.state.currentShot) return;
    const shot = calculateShot(inputData, this.config);
    const keeperDecision = this.keeper.decide({ zone: shot.zone, power: shot.power });
    const startedAt = performance.now();
    this.state.currentShot = { shot, keeperDecision, startedAt };
    this.config.hooks.onShotTaken?.({ shot, keeperDecision });
  ...
  runShotAnimation() {
    const tick = () => {
      ...
      const keeperPose = this.keeper.computePose(keeperDecision, elapsed, shot.durationMs);

      this.renderer.renderBall(ballPos);
      this.renderer.renderKeeper(keeperPose, keeperDecision.direction);

      if (progress >= 1) {
        this.resolveRound(shot, keeperPose, keeperDecision);
        return;
      }
  ...
  resolveRound(shot, keeperPose, keeperDecision) {
    ...
    this.config.hooks.onRoundResolved?.({ round: this.state.round, result, score: this.state.score });
    this.ui.setFeedback(result.goal ? 'GOAL!' : 'Saved by keeper');

    this.state.round += 1;
    this.ui.updateScore(this.state.score, this.config.rounds, Math.min(this.state.round, this.config.rounds));
